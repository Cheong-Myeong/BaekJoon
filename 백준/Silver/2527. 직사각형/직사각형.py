for test_case in range(4):
    ...
    if width_cnt == 0 or height_cnt == 0:  # 포함된 범위가 없다면
        print('d')
    elif width_cnt == 1 and height_cnt == 1:  # 포함된 범위가 가로, 세로 모두 한 지점뿐이라면
        print('c')
    elif width_cnt > 1 and height_cnt > 1:  # 포함된 범위가 가로, 세로 모두 한 지점 초과라면
        print('a')
